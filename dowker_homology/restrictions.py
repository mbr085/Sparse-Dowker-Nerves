...
def Restriction(truncation, restriction_method):
    ...
    **kwargs : dict, optional
    ...
    if isinstance(truncation, truncations.TruncationSparse):
        ...
        isinstance(truncation, truncations.TruncationNone)
        or isinstance(truncation, truncations.TruncationNumpy)
        or isinstance(truncation, truncations.TruncationSheehy)
    ):
...
class RestrictionNumpy(object):
    ...
    def get_restriction_times(self, truncation):
        ...
        self.X = X
        cover_matrix = truncation.get_cover_matrix(X=X, Y=X)
    ...
    def force_monotone_restriction(self):
        ...
        restriction_tree = nerves.get_parent_forest(self.parent_point_list)
        for l in nerves.iterative_topological_sort(restriction_tree, root[0]):
            bucket = list(restriction_tree[l])
            bucket.append(l)
            self.restriction_times[l] = np.max(self.restriction_times[bucket])

    def get_Y_column(self, rf, w):
        if scipy.sparse.issparse(self.truncation.Y):
            return self.truncation.to_numpy_col(
                self.truncation.Y.tocsr()[rf][:, w].tolil()
            )
        else:
                    ...
                    sorted(
                        rf[
                            np.flatnonzero(
                                ...
                                <= np.full_like(
                                    self.restriction_face_indices(l),
...
class RestrictionNone(RestrictionNumpy):
    def get_restriction_times(self, X=None):
        ...
        self.restriction_times = np.full_like(
            self.truncation.truncation_times, np.inf
        )
...
class RestrictionParent(RestrictionNumpy):
    def get_restriction_times(self, X=None):
        ...
        for point_index in range(1, n):
            ...
            max_dist = [
                np.max(Y[truncation_order[parent_index], check_points])
...
class RestrictionSparse(RestrictionNumpy):
    ...
    def l_witnesses(self, l, X):
        if self.restriction_times[l] < np.inf:
            Wl = set(
                list(
                    np.array(self.truncation.Y.rows[l])[
                        np.array(self.truncation.Y.data[l])
                        <= self.restriction_times[l]
                    ]
                )
            )
            Wl.intersection_update(set(list(X.rows[l])))
        else:
    ...
    def get_restriction_times(self, truncation):
        ...
        self.restriction_times = np.full(
            truncation.DD.len(), truncation.DD.max()
        )
        for index in range(len(self.restriction_times)):
            if len(cover_matrix.rows[index]) > 0:
                self.restriction_times[index] = min(cover_matrix.data[index])
            else:
        ...
        self.restriction_times = (
            cover_matrix[
                np.arange(cover_matrix.shape[0]), self.parent_point_list
            ]
            .toarray()
            .flatten()
        )
        self.restriction_times[self.restriction_times == 0] = np.inf
        self.restriction_times[
            self.parent_point_list[np.arange(len(self.parent_point_list))]
            == np.arange(len(self.parent_point_list))
        ] = np.inf
