...
def distance_function(X, dissimilarity):
    if dissimilarity in ["precomputed", "dowker"]:
        ...
        def dist_to_points(point_list):
            return distance.cdist(X[point_list], X, metric=dissimilarity)

    return dist_to_points


def get_farthest_insertion_times(
    X, dissimilarity, initial_point=0, n_samples=None, resolution=0
):
    if n_samples is None:
        n_samples = len(X)
    # initialize
    distances_to_points = distance_function(X, dissimilarity)
    ...
    for point_index in range(1, n_samples):
        ...
        cluster_dist = np.minimum(
            cluster_dist, distances_to_points([farthest_point]).reshape(-1)
        )
...
def Dissimilarity(
    ...
    **kwargs : dict, optional
    ...
    if isinstance(X, np.ndarray):
        ...
        return DissimilaritySparse(X.tolil(), isolated_points=isolated_points)
        ...
        return DissimilaritySparse(C, isolated_points=isolated_points)
    else:
    ...
    **kwargs : dict, optional
    ...
    def set_dissimilarity_from_point_cloud(self, X, metric, **kwargs):
        ...
        self.X = distance.cdist(
            coords[self.farthest_point_list], coords, metric=metric, **kwargs
        )
    ...
    def filtration_value(self, node_indices):
...
class DissimilaritySparse(DissimilarityNumpy):
    def __init__(self, X, isolated_points=True):
        self.isolated_points = isolated_points
        self.shape = X.shape
        self.X = X
        self.X_orig = self.X
    ...
    def filtration_value(self, node_indices):
        ...
        if len(node_indices) == 1:
            if len(self.X.data[node_indices[0]]) == 0:
                return np.inf
            if self.isolated_points:
                return max(0.0, min(self.X.data[node_indices[0]]))
            else:
                return np.inf
        common_witnesses = list(
            set.intersection(*(set(row) for row in self.X.rows[node_indices]))
        )
        ...
        for index in node_indices:
            distances_to_witnesses[self.X.rows[index]] = np.maximum(
                distances_to_witnesses[self.X.rows[index]], self.X.data[index]
            )
        return max(0.0, np.min(distances_to_witnesses[common_witnesses]))
