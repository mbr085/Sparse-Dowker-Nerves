...
class DowkerNerveBase(object):
    ...
    def get_nerve(self, X=None):
        ...
        if self.actual_max_simplex_size() > self.max_simplex_size:
            raise ValueError('Filtered complex will be of size at least ' +
                             ...
                             str(self.actual_max_simplex_size()) + '.')
        return get_nerve_from_maximal_faces(
    ...
    def cardinality_information(self, verbose=True):
        ...
        if hasattr(self, 'nerve'):
            ...
            if verbose:
                print('Nerve has cardinality ',
                      unreduced_cardinality)
...
class AlphaNerve(DowkerNerveBase):
    ...
    def get_maximal_faces(self, X=None):
        ...
        self.maximal_faces = set(
            frozenset(face) for face in tri.simplices)
    ...
    def cardinality_information(self, verbose=True):
        if not hasattr(self, 'DD'):
            if verbose:
                print('No data added')
            return {}
        cardinality = {'unreduced_cardinality': np.sum([
            scipy.special.binom(self.DD.len(), _i)
            for _i in range(1, self.dimension + 3)])}
        if verbose:
            print('Unreduced nerve has cardinality ' + str(
                cardinality['unreduced_cardinality']))
        if hasattr(self, 'nerve'):
            cardinality['reduced_cardinality'] = len(self.nerve)
            if verbose:
                print('Reduced nerve has cardinality ' + str(
                    cardinality['reduced_cardinality']))
            ...
            if verbose:
                print('The ' +
                      ...
                      'has cardinality ' + str(cardinality['maximal_faces']))
        cardinality['dimension'] = self.max_cardinality - 1
        if verbose:
            print('The simplicial complex has dimension',
...
class DowkerNerve(DowkerNerveBase):
    ...
    def get_maximal_faces(self, X=None):
        ...
        self.slope = get_slope(restriction.parent_point_list,
                               restriction.restriction_times)
        ...
        for l in np.argsort(restriction_times)[::-1]:
            ...
            for w in restriction.l_witnesses(l, self.DD.X):
                ...
                slope_vertices = restriction.slope_vertices(
                    self.slope, w, rf)
            ...
            l_faces = eliminate_subsets(
                list(frozenset(l_faces)))
            maximal_faces = maximal_faces.union(l_faces)
        self.maximal_faces = eliminate_subsets(
            list(maximal_faces))
...
class DowkerNerveAmbient(DowkerNerve):
    ...
    def set_dowker_dissimilarity(self, X):
        ...
        self.DD = dissimilarities.DissimilarityAmbient(
            X, resolution=self.resolution, n_samples=self.n_samples,
            initial_point=self.initial_point)
        self.set_translation_function()
    ...
    def interleaving_line(self, n_points=100):
        ...
        x_points = np.linspace(0, 1.01 * xmax, n_points)
        y_points = np.maximum(
            0.5 * self.translation_function(
                2 * (x_points + self.DD.cover_radius)),
            x_points + self.truncation.offset)
        y_points[x_points > xmax] = ymax
        return x_points, y_points
    ...
    def get_filtered_nerve(self, X=None):
        ...
        finite_faces = np.array([
            self.nerve_values[index] <= np.min(
                self.restriction.restriction_times[list(simplex)])
            for index, simplex in enumerate(self.nerve)])
        self.nerve = [simplex for index, simplex in enumerate(
            self.nerve) if finite_faces[index]]
...
def get_slope(parent_point_list, death_times):
    ...
    for index in range(len(parent_point_list)):
        if (len(forest[index]) > 0 and
                death_times[index] < np.inf):
