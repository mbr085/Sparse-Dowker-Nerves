...
class Persistence(object):
    ...
    **kwargs : dict, optional
    ...
    def check_arguments(self):
        ...
        if self.truncation_method == "Sheehy":
        ...
        # Sheehy restriction
        if self.restriction_method == "Sheehy":
        ...
        # truncation methods
        truncation_methods = {"Sheehy", "dowker", "Canonical", "no_truncation"}
        if (
            self.truncation_method is not None
            and self.truncation_method not in truncation_methods
        ):
            raise ValueError(
                "Allowed values for truncation_method are", truncation_methods
            )
        # alpha complex
        if self.restriction_method == "alpha" and self.dissimilarity not in {
            "alpha",
            None,
        }:
            raise ValueError(
    ...
    def set_defaults(self):
        if self.truncation_method is None:
            if self.restriction_method == "no_restriction":
    ...
    def set_nerve(self, X):
        ...
        if (
            self.restriction_method == "no_restriction"
            and self.truncation_method == "no_truncation"
        ):
    ...
    def get_simplex_tree(self, X=None):
        ...
        self.simplex_tree = SimplexTreeFromNerve(self.nerve, self.coeff_field)
        return self.simplex_tree
    ...
    def components(
        self, X=None, persistence_function=0, max_birth_value=np.inf
    ):
        self.persistent_homology(X)
    ...
    def cycle_representatives(self, X=None, persistence_function=0):
        ...
        return [
            [self.nerve.truncation.point_sample[list(edge)] for edge in cycle]
            for cycle in cycles
        ]
